...
- (void) execute
{
	...
	{
    NSData* data = [[self->m_pipeOut fileHandleForReading] availableData];
		if ([data length] > 0)
		{
      if (self->m_delegate)
			{
        if ([self->m_delegate respondsToSelector:@selector(outputData:providedByCommand:)])
				{
          [self->m_delegate outputData:data providedByCommand:self];
				}
			}
			
      [[self->m_pipeOut fileHandleForReading] waitForDataInBackgroundAndNotify];
		}
	}];
	...
	{
    NSData* data = [[self->m_pipeError fileHandleForReading] availableData];
		if ([data length] > 0)
		{
      if (self->m_delegate)
			{
        if ([self->m_delegate respondsToSelector:@selector(errorData:providedByCommand:)])
				{
          [self->m_delegate errorData:data providedByCommand:self];
				}
			}
			
      [[self->m_pipeError fileHandleForReading] waitForDataInBackgroundAndNotify];
		}
	}];
    ...
    if (self->m_delegate)
		{
      if ([self->m_delegate respondsToSelector:@selector(commandDidFinish:withExitCode:)])
			{
        [self->m_delegate commandDidFinish:self withExitCode:[self->m_task terminationStatus]];
			}
		}
	}];
